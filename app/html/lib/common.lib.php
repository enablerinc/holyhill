...
// 회원 프로필 이미지 URL 가져오기 (jpg 우선, 없으면 gif)
function get_member_profile_url($mb_id) {
    if (!$mb_id) return '';
    ...
    if (file_exists($jpg_path)) {
        return G5_DATA_URL . '/member_image/' . $mb_dir . '/' . $mb_id . '.jpg';
    }

    // .gif 파일 확인 (기존 업로드)
    $gif_path = G5_DATA_PATH . '/member_image/' . $mb_dir . '/' . $mb_id . '.gif';
    if (file_exists($gif_path)) {
        return G5_DATA_URL . '/member_image/' . $mb_dir . '/' . $mb_id . '.gif';
    }
